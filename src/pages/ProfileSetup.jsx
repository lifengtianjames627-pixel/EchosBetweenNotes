...
export default function ProfileSetup() {
  ...
  return <div className="min-h-screen px-4 py-12" style={{ background: '#f3efe6' }}><div className="max-w-xl mx-auto"><SetupHeader /><form onSubmit={e => { e.preventDefault(); save.mutate(); }} className="p-5 sm:p-7" style={{ background: '#faf8f2', border: '1px solid #e0d8c8', borderRadius: 12 }}><MusicPreferences values={preferences} onToggle={toggle} /><ProfileEssentials biography={biography} onChange={setBiography} /><ConnectionChoices locationAllowed={locationAllowed} hasBand={hasBand} bandName={bandName} onChange={change} /><SetupSubmit pending={save.isPending} /></form></div></div>;
}